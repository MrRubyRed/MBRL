...
rms = tf.sqrt(tf.reduce_mean(tf.reduce_sum(tf.square(tf.subtract(dyn.next_state_, dyn.output_)), 1, keep_dims=True)))
learning_rate = 0.01
momentum = 0.995
train_step = tf.train.MomentumOptimizer(learning_rate=learning_rate, momentum=momentum).minimize(rms)
