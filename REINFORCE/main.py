...
def main_cartpole(n_iter=100, gamma=1.0, min_timesteps_per_batch=1000, stepsize=1e-2, animate=True, logdir=None):
    ...
    sy_logprob_n = fancy_slice_2d(sy_logp_na, tf.range(sy_n), sy_ac_n) # this fetches the logprobability of the actions taken -- used for policy gradient calculation
...
def main_pendulum(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_type, vf_params, animate=False):
    ...
    h2 = lrelu(dense(sy_ob_no, 32, "h1", weight_init=normc_initializer(1.0))) # hidden layer
    mean_na = dense(h2, ac_dim, "final", weight_init=normc_initializer(0.1)) #VRR Mean control output
    logstd_a = tf.get_variable("logstdev", [ac_dim], initializer=tf.zeros_initializer()) #VRR 
    std_a = tf.exp(logstd_a);
    # we use a small initialization for the last layer, so the initial policy has maximal entropy
    #sy_na = tf.placeholder(shape=[None, num_actions], name='oldlogits', dtype=tf.float32) # logits BEFORE update (just used for KL diagnostic)
    #sy_logp_na = tf.nn.log_softmax(sy_logits_na) # logprobability of actions
    #sy_sampled_ac = categorical_sample_logits(sy_logits_na)[0] # sampled actions, used for defining the policy (NOT computing the policy gradient)
    sy_n = tf.shape(sy_ob_no)[0]
    #sy_logprob_n = fancy_slice_2d(sy_logp_na, tf.range(sy_n), sy_ac_n) # this fetches the logprobability of the actions taken -- used for policy gradient calculation

    sy_sampled_ac = mean_na + std_a*tf.random_normal(tf.shape(mean_na));
    sy_logprob_n = -tf.log(tf.sqrt(2.0*np.pi))-logstd_a-tf.divide(tf.square(sy_ac_n-mean_na),tf.square(std_a));

    #VRR The following quantities are just used for computing KL and entropy, JUST FOR DIAGNOSTIC PURPOSES >>>>
    sy_oldlogp_na = tf.placeholder(shape=[None, ac_dim], name='oldlogits', dtype=tf.float32);#-tf.log(tf.sqrt(2.0*np.pi))-logstd_a-tf.divide(tf.square(sy_na-mean_na),tf.square(std_a));
    sy_oldp_na = tf.exp(sy_oldlogp_na)
    sy_logp_na = -tf.log(tf.sqrt(2.0*np.pi))-logstd_a-tf.divide(tf.square(sy_ac_n-mean_na),tf.square(std_a));
    sy_kl = tf.reduce_sum(sy_oldp_na * (sy_oldlogp_na - sy_logp_na)) / tf.to_float(sy_n)
    sy_p_na = tf.exp(sy_logp_na)
    sy_ent = tf.reduce_sum( - sy_p_na * sy_logp_na) / tf.to_float(sy_n)   
    #
    ...
    for i in range(n_iter):
        ...
        while True:
            ...
            while True:
                if animate_this_episode:
                    env.render()
                obs.append(ob)
                ac = sess.run(sy_sampled_ac, feed_dict={sy_ob_no : ob.T})
                acs.append(ac)
                ob, rew, done, _ = env.step(ac)
                if(ob.ndim == 1): ob = ob[None].T;
                #ob = ob.T;
                rewards.append(rew)
                if done:
                    break                    
        ...
        for path in paths:
            rew_t = path["reward"]
            return_t = discount(rew_t, gamma)
            if(return_t.ndim == 1): return_t = return_t[None].T;
            vpred_t = vf.predict(path["observation"][:,:,0])
            adv_t = return_t - vpred_t
            advs.append(adv_t)
            vtargs.append(return_t)
            vpreds.append(vpred_t)
        ...
        # Build arrays for policy update
        ob_no = np.concatenate([path["observation"] for path in paths])[:,:,0]
        ac_n = np.concatenate([path["action"] for path in paths])[:,:,0]
        adv_n = np.concatenate(advs)
        standardized_adv_n = (adv_n - adv_n.mean()) / (adv_n.std() + 1e-8)
        vtarg_n = np.concatenate(vtargs)
        vpred_n = np.concatenate(vpreds)
        ...
        # Policy update
        _, old_logp_na = sess.run([update_op, sy_logprob_n], feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_adv_n:standardized_adv_n, sy_stepsize:stepsize})
        kl, ent = sess.run([sy_kl, sy_ent], feed_dict={sy_ob_no:ob_no, sy_ac_n:ac_n, sy_oldlogp_na:old_logp_na})
...
if __name__ == "__main__":
    ...
    if 1:
        general_params = dict(gamma=0.97, animate=True, min_timesteps_per_batch=2500, n_iter=1000, initial_stepsize=1e-3)
        params = [
        ...
        main_pendulum(logdir='/tmp/ref/linearvf-kl2e-3-seed0', seed=0, desired_kl=2e-3, vf_type='linear', vf_params={}, **general_params)
