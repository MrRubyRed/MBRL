...
for j in range(T_hor):
    a = np.random.uniform(-2.0,2.0,(1,));
    in_nn[0,:] = s;
    s_n_nn = sess.run(Big_l[0],{input_s:in_nn,u_l:np.concatenate((a[None],np.zeros((1,H_depth-1))),axis=1)});# np.array([[a[0]]])});
    s_n_env = env.step(a);
    print("L_2 difference: " + str(np.linalg.norm(s_n_nn - s_n_env[0])));
    s = s_n_nn;
